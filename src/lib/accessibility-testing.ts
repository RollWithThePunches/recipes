...
export const nvdaTestScenarios = {
  ...
  async testNVDAAnnouncement(
    ...
    expect(announcement.toLowerCase()).toContain(expectedAnnouncement.toLowerCase());
  },
...
export const renderWithA11y = (ui: ReactElement, options = {}) => {
  ...
  return {
    ...
    testScreenReader: (expectedText: string) => {
      const { container } = utils;
      return screenReaderTestUtils.testScreenReaderAnnouncement(container, expectedText);
    },
    testNVDA: (expectedAnnouncement: string) => {
      const { container } = utils;
      return nvdaTestScenarios.testNVDAAnnouncement(container, expectedAnnouncement);
    }
  };
};
